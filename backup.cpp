...
bool countList(Node *pTemp,int counter = 0){
    if (pTemp == NULL){
        return false;
    }
    if(counter == 0){
    ...
    if(countList(pTemp->pNext,++counter)){
        std::cout << "->";
    }
    std::cout << counter;
    if(counter == 1){
	...
	while (window.isOpen()&&(!boardFull(board))&&(!maxGoal(board,squaresPerSide) || byPass))
	{
        ...
        for(int i = 0; i < squaresPerSide; i++){
            for(int j = 0; j < squaresPerSide; j++){
                ...
                squaresArray[i*squaresPerSide+j] = Square(50,50 * j + j * 10, 50 * i + i * 10 , sf::Color::Blue, true, nameBoard);
