...
int main()
{	
	const int numberOfPiecesPerSide = 4;
	char aString[ 81];    // used to create a string to display text in a square 
	int board[ numberOfPiecesPerSide];        // Board of 4 integer values
	...
	for( int i=0; i<numberOfPiecesPerSide * numberOfPiecesPerSide; i++) {
		board[ i] = 0;
	}
	// Set two randomly selected board positions to have a value 2 or 4 in it.
	placeRandomPiece( board, numberOfPiecesPerSide);
	placeRandomPiece( board, numberOfPiecesPerSide);
	
	// Create array of Square objects
	int arraySize = numberOfPiecesPerSide * numberOfPiecesPerSide;
	...
	for( int i=0; i<arraySize; i++) {
		...
		if( board[ i] == 0) {
		   ...
		   sprintf( name, "%d", board[ i]);   // "print" the square creation order number into a string 
